...
Cloud Run images are owned by the application delivery flow rather than Terraform. When commit-addressed application images exist, redeploy the last known-good commit image and verify the public `/health` route; do not use `latest`.
